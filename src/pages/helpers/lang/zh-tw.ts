export const content = {
  ...
  index: {
    ...
    box3: {
      title: '掌控你數據的所有權',
      p: [
        '用戶可以利用種子網絡自建服務、應用、社交群組，完全掌控自己的身份數據以及創造的內容數據，無需依賴中心化平台提供服務。',
    ...
    box4: {
      title: '隱私保護',
      p: [
        '用戶與應用的交互、與其它用戶的通訊，在 RumNetwork 上 是通過專屬區塊鏈上的加密消息傳遞，在應用裡解密並回放。 Rum 不存在也不可能對任何用戶數據及行為進行收集、追踪、監聽。',
        '用戶的隱私權將將通過加密與匿名自主性來保證。',
      ],
    },
    box5: {
      title: '用戶的隱私權將將通過加密與匿名自主性來保證。',
      p: [
        '當所有數據均為用戶掌握，內容創造和價值交換都不再需要傳統中介機構充當金錢和信息的仲裁者。用戶可以通過智能合約為自己創建的內容定價，或出售群組門票等，之後也可以在此基礎上參與流量分發，加速內容交易，由此獲得分發傭金。',
      ],
      // link: '关联钱包，开始挖矿',
    },
    box6: {
    ...
    howWorks: {
      ...
      p: [
        ...
        [{ type: [], text: '私有鏈+加密消息網絡+虛擬貨幣+智能合約+數字簽名，讓RumSystem 和基於種子網絡的人群聚合 具備潛力無限的應用形式，可以成為協作社群或商業應用，如 共創、內容交易，收費入會 等，也可以只用於小圈子私密社交、個人多節點數據備份，可接入開放網絡，也可以只運行在私密網絡上。交易場景中，可以參與 RUM token 的經濟生態，也可無幣化運營。' }],
      ],
      viewLarger: '點擊查看大圖',
    },
    ...
    terms: [
      [
        ...
        { name: 'GROUP (SEEDNET): ', text: '多個或單個節點可組建種子網絡。每個種子網絡可以設定為不同的產品形態，呈現為功能各異的在線應用：如去中心化版本的 Twitter、Reddit、付費群組、付費閱讀、甚至一個僅在自己搭建的多個設備節點間同步的私密筆記。' },
      ...
      [
        { name: 'EVENT/MESSAGE: ', text: '用戶在種子網絡裡提交的任何操作都被視為一個事件，例如發布新貼、更新頭像和留言回复。每個事件將作為交易廣播給 Group Owner 以及所有出塊人，等待記錄到新塊中。 RUM 使用 Activity Vocabulary Core Types 的子集來表示事件類型。' },
  ...
  apps: {
    ...
    content: [
      ...
      '支持所有主流 desktop OS 和Android，瀏覽器版本正在開發中。',
      '目前，Rum App 支持三類種子網絡模板：Feed,BBS,私密筆記。即將支持虛擬幣經濟系統、智能合約、內容確權等，讓種子網絡可以支持協作社群或更多付費交易場景。',
      '不久的將來，結合不同的權限設置、經濟關係設定、主題定制，一個種子網絡可以化身為去中心的 Twitter、Reddit、付費群組、私密筆記、加密信託文檔、私密群組社交、內容眾創平台、NFT、內容交易服務、自出版服務……',
    ],
  ...
  why: {
    ...
    content: [
      ...
      '將用戶隱私當商品、不尊重作者的價值、追踪用戶行為和社交關係、使用算法控制分發、操控信息的生產和消費、影響用戶的“獨立思考結果”，這些對傳統互聯網平台的批評，在各個國家早已是主流聲音。對於內容創作而言，佔據壟斷地位的內容中介也可利用其支配地位，為迎合商業化目標，粉碎意見和品位的多樣性。市場高度集中後的面臨的就是同質化，這也是我們正目睹的現實。',
  ...
  network: {
    ...
    box1: {
      title: '為數字內容交易而設計的生態',
      content: [
        '去中心的 Rum Network 可令數字內容的交易無須再依賴傳統中介機構充當金錢和信息的仲裁者。用戶可以通過智能合約為自己創建的內容定價，或出售群組門票等，之後也可以在此基礎上參與流量分發，加速內容交易，由此獲得分發傭金。',
      ],
      link: '關聯錢包，開始挖礦（開發中）',
    },
    box2: {
  ...
  developers: {
    ...
    rumApp: {
      ...
      desc: [
        ...
        'RumNetwork 裡所有用戶操作請求和通訊都是通過私有鍊和加密消息傳遞。用戶通過創建或加入種子網絡進行聚合、社交及互動。根據不同的需求，每個種子網絡都可渲染為（render）不同的產品呈現（interface）。 Interface 可以是一個應用，或是一種模版/主題。',
        '基於這個設計框架，第三方開發者可以根據你的想法設計全新的產品呈現；也可為 RumApp 設計新的模板、主題、插件，探索去中心應用及社交的無限可能。',
        '我們即將在 RumApp 裡支持虛擬幣經濟系統、智能合約、內容確權等，讓種子網絡可以支持協作社群或更多付費交易場景。',
      ],
      links: {
        start: 'Start',
        docs: 'Docs',
