export const content = {
  ...
  index: {
    ...
    box2: {
      title: '去中心化',
      p: [
        '在 RumNetwork 中，用戶可自建種子網絡 。每個種子網絡通過一條專屬的私有區塊鏈進行通訊及傳遞數據。',
        '數據經加密後在網絡成員中同步。數據的保管或分發無需通過中心化的服務提供商。',
      ],
      link: '加入併成為節點',
    },
    box3: {
  ...
  apps: {
    ...
    feed: {
      ...
      content: [
        '在瀏覽器運行的輕節點，簡單輕巧，毫無系統負擔。',
        '超低進入門檻，使用常用的社交平台賬戶就可以登入。',
        '超低學習成本，用戶熟悉的內容發佈方式和交互方式',
        '算法推薦不同種子網絡的熱門內容，方便用戶找到自己的興趣所在。',
      ],
    },
    quorum: {
      title: 'Quorum',
  ...
  developers: {
    ...
    quorum: {
      ...
      desc: [
        ...
        '我們的協議、API 和 SDK 還在不斷完善中。歡迎持續關注我們的進展，提出你的需求和想法。或加入 RumNetwork 體驗網絡，成為我們的出塊節點。',
      ],
      links: {
        api: 'API 文檔',
        github: 'GitHub 倉庫',
    ...
    rumApp: {
      title: 'Rum App',
      desc: [
        'Rum App 是官方團隊開發的開源跨平台客戶端。',
        '我們開發 Rum App 的目的之一是結合現實應用場景對 Quorum 的 SDK 及 API 進行測試及驗證。向有意基於 RumSystem 開發去中心應用的第三方開發者提供設計思路。',
        '在Rum App裡，一個種子網絡可以被視為一個社交群組，一個私密推特廣場，一個blog，一個論壇版塊，一個收費社群。通過不同的模板、主題、佈局涉及、權限及經濟系統的設置的組合。每個種子網絡都可渲染為（render）不同的產品呈現（interface）。',
        '基於這個設計框架，第三方開發者可以根據你的想法設計全新的產品呈現；也可為 Rum App 設計新的模板、主題、插件，探索去中心應用及社交的無限可能。',
        '我們即將在 Rum App 裡支持虛擬幣經濟系統、智能合約、內容確權等，讓種子網絡可以支持協作社羣或更多付費交易場景。',
      ],
      links: {
        start: 'Start',
        docs: 'Docs',
